...
int print_right(char *str, han_s *handler)/*prints in a normal alignment*/
{
	...
	/*numberto be padded with zeroes left align not invokade*/
	/*to invoke zero loading fill*/
		...
		if ((handler->plus && !neg1) || (!handler->plus && hand->space && !neg1))
			len++;
...
/**
 *print_left - print a no, with left alignement invoked
 *@str:number
 *@handler:struct
